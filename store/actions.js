...
export default {
  ...
  /**
   * Updates the statistics of passed workout
   */
  updateWorkoutStatistics ({state}, workout) {
    if (!workout) { return }

    const id = workout.key
    ...
    let updates = {}
    updates['/statistics/' + state.user.uid + '/doneWorkouts/' + id] = deleteKey(workout)
